...
export default function ComprarBacklinksAlimentacao() {
  ...
  const filtered = useMemo(() => {
    ...
    return (backlinks ?? []).filter((b) => {
      // Restrito a Alimentação
      const cat = normalize(String(b.category ?? ""));
      if (!(cat === "alimentacao")) return false;
      ...
      <StructuredData
        ...
        data={{
          items: [
            ...
            {
              name: "Backlinks de Alimentação",
              url: `${window.location.origin}/comprar-backlinks-alimentacao`,
            },
          ],
        }}
      />
      ...
      <main className="container mx-auto px-4 py-28 grid grid-cols-1 md:grid-cols-12 gap-8">
        ...
        <section className="md:col-span-10">
          <Breadcrumbs
            ...
            items={[
              { name: "Início", url: "/" },
              { name: "Backlinks de Alimentação", url: "/comprar-backlinks-alimentacao" },
            ]}
          />
          <h1 className="text-4xl font-bold mb-6">Backlinks no Nicho de Alimentação</h1>
